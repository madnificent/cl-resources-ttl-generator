...
(defun make-resource-shape (resource)
  ...
  (format nil "~{~@[~A~%~]~}~%"
          `(,(resource-shape-uri resource)
            ...
            ;; the properties
            ,(when (mu-cl-resources::ld-properties resource)
               (format
                nil
                "~@[  sh:property [~{~&~A~%~^  ] , [~}  ];~]"
                (loop for slot in (mu-cl-resources::ld-properties resource)
                      ...
                      (format
                       nil
                       "    sh:path ~A;~%    sh:minCount ~A~@[;~%    sh:maxCount ~A;~]~@[;~%    sh:dataType ~A~]"
            ...
            ;; the relations
            ,(when (mu-cl-resources::all-links resource)
               (format
                nil
                "~@[  sh:property [~{~&~A~%~^  ] , [~}  ];~]"
                (loop for link in (mu-cl-resources::all-links resource)
                      ...
                      (format
                       nil
                       "    sh:path ~:[~A~;[ sh:inversePath ~A]~];~%    sh:minCount 0~@[;~%    sh:maxCount ~A~];~%    sh:class ~A;~%    sh:nodeKind sh:IRI"
                       (mu-cl-resources::inverse-p link)
